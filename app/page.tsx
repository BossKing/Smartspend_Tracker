...
import { Trash2, Edit2, Plus, Moon, Sun, BarChart3, Zap } from 'lucide-react';
  ...
  const handleAddExpense = () => {
    ...
    if (editingId) {
      setExpenses(expenses.map(exp => 
        exp.id === editingId ? { ...formData, id: editingId, amount: parseFloat(formData.amount) } : exp
      ));
      setEditingId(null);
    } else {
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
                ...
                <div className={`rounded-lg border p-6 ${cardClass}`}>
                  ...
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={trendData}>
                      ...
                      <Tooltip contentStyle={{ backgroundColor: darkMode ? '#1e293b' : '#f8fafc', border: `1px solid ${darkMode ? '#334155' : '#e2e8f0'}` }} formatter={(value) => formatCurrency(value)} />
        ...
        {currentPage === 'expenses' && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <button onClick={() => { setShowForm(!showForm); setEditingId(null); setFormData({ title: '', amount: '', category: 'Other', date: new Date().toISOString().split('T')[0], description: '' }); }} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 flex items-center gap-2">
            ...
            {showForm && (
              <div className={`rounded-lg border p-6 ${cardClass}`}>
                <div className="grid md:grid-cols-2 gap-4 mb-4">
                  <input type="text" placeholder="Title" className={`px-4 py-2 rounded-lg border ${darkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-300'} focus:outline-none focus:ring-2 focus:ring-blue-500`} value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} />
                  <div className="flex gap-2">
                    <input type="number" step="0.01" placeholder="Amount" className={`flex-1 px-4 py-2 rounded-lg border ${darkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-300'} focus:outline-none focus:ring-2 focus:ring-blue-500`} value={formData.amount} onChange={(e) => setFormData({ ...formData, amount: e.target.value })} />
                    <div className={`px-3 py-2 rounded-lg border ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-100 border-slate-300'} flex items-center font-semibold`}>{currency === 'INR' ? '₹' : '$'}</div>
                  </div>
                  <select className={`px-4 py-2 rounded-lg border ${darkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-300'} focus:outline-none focus:ring-2 focus:ring-blue-500`} value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })}>{CATEGORIES.map(cat => <option key={cat} value={cat}>{cat}</option>)}</select>
                  <input type="date" className={`px-4 py-2 rounded-lg border ${darkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-300'} focus:outline-none focus:ring-2 focus:ring-blue-500`} value={formData.date} onChange={(e) => setFormData({ ...formData, date: e.target.value })} />
                </div>
                <textarea placeholder="Description (optional)" className={`w-full px-4 py-2 rounded-lg border ${darkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border-slate-300'} focus:outline-none focus:ring-2 focus:ring-blue-500 mb-4 resize-none`} rows={3} value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })} />
                <div className="flex gap-2">
                  <button onClick={handleAddExpense} className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700">{editingId ? 'Update' : 'Add'} Expense</button>
                  <button onClick={() => { setShowForm(false); setEditingId(null); setFormData({ title: '', amount: '', category: 'Other', date: new Date().toISOString().split('T')[0], description: '' }); }} className={`px-4 py-2 rounded-lg ${darkMode ? 'bg-slate-800 hover:bg-slate-700' : 'bg-slate-100 hover:bg-slate-200'}`}>Cancel</button>
                </div>
              </div>
            )}
              ...
              {expenses.length === 0 ? (
                ...
                expenses.map(expense => (
                  <div key={expense.id} className={`rounded-lg border p-4 flex items-center justify-between ${cardClass}`}>
                    ...
                    <div className="flex gap-2">
                      <button onClick={() => handleEditExpense(expense)} className="p-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white"><Edit2 size={16} /></button>
                      <button onClick={() => handleDeleteExpense(expense.id)} className="p-2 rounded-lg bg-red-600 hover:bg-red-700 text-white"><Trash2 size={16} /></button>
                    </div>
                  </div>
                ))
              )}
        ...
        {currentPage === 'insights' && (
          <div className="space-y-6">
            ...
            <button onClick={handleGenerateInsight} disabled={loadingInsight || expenses.length === 0} className="bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
